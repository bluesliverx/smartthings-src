...
class UPNPHTTPServerHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path == '/lights_wsd.xml':
            self.send_response(200)
            self.send_header('Content-type', 'application/xml')
            self.end_headers()
            self.wfile.write(self.get_wsd_xml().encode())
            return
        if self.path == '/ltech-wifi-104.xml':
            self.send_response(200)
            self.send_header('Content-type', 'application/xml')
            self.end_headers()
    ...
    <device>
        <deviceType>urn:schemas-upnp-org:device:LTechWiFi104:1</deviceType>
        ...
        <serviceList>
            <service>
                <URLBase>http://xxx.yyy.zzz.aaaa:1050</URLBase>
                <serviceType>urn:ltech.example.com:service:LTechWiFi104:1</serviceType>
                <serviceId>urn:ltech.example.com:serviceId:LTechWiFi104</serviceId>
                <controlURL>/lights</controlURL>
                <eventSubURL/>
                <SCPDURL>/lights_wsd.xml</SCPDURL>
            </service>
        </serviceList>
